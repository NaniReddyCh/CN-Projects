...
class iplb (object):
  ...
  def __init__ (self, connection, service_ip, servers = []):
    ...
    self.resp_time = [0 for _ in self.servers]
    self.pointer = 0
    self.counter = []
    self.copy_counter = []
    for i in range(len(self.resp_time)):
        self.resp_time[i] = int(input(f"Enter the resp time for {i + 1}th server"))
    factor = 1
    for t in self.resp_time:
      factor *= (t / 10)
    for t in self.resp_time:
      self.counter.append(factor / (t / 10))
    for ct in self.counter:
      self.copy_counter.append(ct)
  ...
  def _pick_server(self, key, inport):
    """
    Pick a server for a (hopefully) new connection based on response time.
    """
    if len(self.live_servers) < 2:
        log.warn("Not enough servers available to check response time.")
        return random.choice(list(self.live_servers.keys()))
    if (self.counter)[(self.pointer)] == 0:
        (self.counter)[(self.pointer)] = (self.copy_counter)[(self.pointer)]
        (self.pointer) = ((self.pointer) + 1) % len(list(self.live_servers.keys()))
    (self.counter)[(self.pointer)] -= 1
    return list(self.live_servers.keys())[(self.pointer)]
